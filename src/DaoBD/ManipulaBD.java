...
public class ManipulaBD {
	...
	private static ManipulaBD manipulaBDinstance;
	private SQLiteDatabase database;
	private static CriaBD criaBD;

	private ManipulaBD() {

	}

	public static ManipulaBD getInstance(Context context) {
		if (manipulaBDinstance == null) {
			manipulaBDinstance = new ManipulaBD();
			criaBD = new CriaBD(context);
		}
		return manipulaBDinstance;
		...
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			if (!(columnArray1.contains(cursor.getString(1)))) {
				columnArray1.add(cursor.getString(cursor
						.getColumnIndex(CriaBD.NOME_ATIVIDADE)));
			}
	...
	public Atividade getAtividadePorNome(String nomeAtividade) {
		...
		String[] arrayColunas = {
				CriaBD.NOME_ATIVIDADE,
				"sum(" + CriaBD.TEMPO_ATIVIDADE + ") as "
						+ CriaBD.TEMPO_ATIVIDADE, CriaBD.DATA_ATIVIDADE,
				CriaBD.PRIORIDADE_ATIVIDADE };
		String[] arrayArgs = {/* CriaBD.NOME_ATIVIDADE, */nomeAtividade };
		// Novamente, se colocando o nome da coluna do where por parametro ele n
		// funciona
		Cursor cursor = database.query(CriaBD.TABLE_NAME, arrayColunas,
				"nome = ?", arrayArgs, null, null, null);
		SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
		Atividade atv = null;
		// Se não ta vazio
		if (cursor.moveToFirst()) {
			String nome = cursor.getString(cursor
					.getColumnIndex(CriaBD.NOME_ATIVIDADE));
			int tempo = cursor.getInt(cursor
					.getColumnIndex(CriaBD.TEMPO_ATIVIDADE));
			String prioridade = cursor.getString(cursor
					.getColumnIndex(CriaBD.PRIORIDADE_ATIVIDADE));
			Date data = null;
			try {
				data = format.parse(cursor.getString(cursor
						.getColumnIndex(CriaBD.DATA_ATIVIDADE)));
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			atv = new Atividade(nome, tempo, data, prioridade);
		}
		return atv;
	}

	public List<Atividade> getAtividadesDaSemana(String dataInicio,
			String dataFim) {
		database = criaBD.getReadableDatabase();
		SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
		...
		Cursor cursor = database.query(CriaBD.TABLE_NAME, arrayColunas,
				"data between ? and ?", arrayArgs, null, null, null);
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			String nome = cursor.getString(cursor
					.getColumnIndex(CriaBD.NOME_ATIVIDADE));
			int tempo = cursor.getInt(cursor
			...
			Date data = null;
			try {
				data = dbFormat.parse(cursor.getString(cursor
						.getColumnIndex(CriaBD.DATA_ATIVIDADE)));
			} catch (ParseException e) {
			}
			Atividade atv = new Atividade(nome, tempo, data, prioridade);
